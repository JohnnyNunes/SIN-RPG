...
(function(_, $) {
	...
	TimedAttackSystem.prototype.playCircleGame = function() {
	    ...
	    if(eval($.activateCondition) && this._notPressed) {
	    	this._notPressed = false;
			AudioManager.playSe({"name":this._item.se,"pan":0,"pitch":100,"volume":40});
			var x_bk = this._xPosition;
			this._xPosition -= 50;
			if(this._xPosition <= 0) this._xPosition = this._xPosition + ((50 - this._xPosition)*2);
			var entireLength = this.width;
			var power = 1 - (this._xPosition / (this._radius - 50));
			this.setPower(power);
			this._xPosition = x_bk;
	    }
